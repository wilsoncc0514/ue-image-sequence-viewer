...
- 左右侧栏采用一致的初始宽度，移除文件树无意义的横向滚动条。
- 标签文本框默认单行，并随自动换行后的可见行数增长或收缩。
- 双击图片可在系统文件管理器中打开所在目录并选中文件。
- 展开序列时以可中断动效将目标项滚动到视区中部，并支持减少动态效果。
- 增加 macOS 与 Windows 双击启动器，收敛运行日志并补齐跨平台 CI。
- 全项目 mypy 检查已收敛为零错误，并完善 GUI、交互和生命周期回归测试。
