        ...
        try:
            with open(import_path, 'r', encoding='utf-8-sig') as csvfile:
                ...
                if not reader.fieldnames or 'seq' not in reader.fieldnames:
                    if show_message:
                        messagebox.showerror('格式错误', "导入的 CSV 缺少 'seq' 列，无法匹配序列。")
                    return None
                seq_map = {self._normalize_seq(k): k for k in self.tag_data.keys()}
                if '.' in self.tag_data:
                ...
                for row in reader:
                    ...
                    if not seq_val or seq_val == 'Unknown':
                        continue
                    status_val = self._csv_text(row.get('状态（合格/不合格/待定）') or row.get('状态（合格/不合格）'))
                    ...
                    if qc_val and (not qc_by_found):
                        qc_by_found = qc_val
                    if not status_val and (not str(light_str).strip()) and (not str(comp_str).strip()) and ('重新渲染' not in str(remark_str)):
                        continue
                    ...
                    if not target_rel_path:
                        ...
                        continue
                    matched_count += 1
                    state = self._create_empty_tag_state(status=status_val, rerender='重新渲染' in str(remark_str))
                    ...
                    if self._is_tag_state_empty(state):
                        continue
                    self.tag_data[target_rel_path] = state
                if qc_by_found:
                    self.qc_by_name = qc_by_found
            ...
            if show_message:
                messagebox.showinfo('导入成功', f'已成功读取并同步 CSV 标记数据。\n匹配：{matched_count} 条；未匹配：{unmatched_count} 条。')
            return {'matched': matched_count, 'unmatched': unmatched_count, 'qc_by': qc_by_found}
